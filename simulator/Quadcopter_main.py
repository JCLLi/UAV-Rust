...
yaw_controller = PID_Controller(Kp_ang, Kd_ang, Ki_ang, Ki_sat_ang, dt)
...
#     angle[0].append((quadcopter.angle[0]))
#     angle[1].append((quadcopter.angle[1]))
#     angle[2].append((quadcopter.angle[2]))

#     angle_vel[0].append((quadcopter.ang_vel[0]))
#     angle_vel[1].append((quadcopter.ang_vel[1]))
#     angle_vel[2].append((quadcopter.ang_vel[2]))
...
for time in enumerate(time_index):
    ...
    ang_error = quadcopter.calc_ang_error(quadcopter.angle)
    ang_vel_error = quadcopter.calc_ang_vel_error(quadcopter.ang_vel)
    tau_needed = yaw_controller.control_update(ang_error, ang_vel_error)
    #Find motor speeds needed to achieve desired linear and angular accelerations
    quadcopter.des2speeds(10, tau_needed)
    # Step in time and update quadcopter attributes
    quadcopter.step()
...
def total_plot():
    ...
    axes.plot(time_index, velocity[0], label= 'u')
    axes.plot(time_index, velocity[1], label= 'v')
    axes.plot(time_index, velocity[2], label= 'w')
    ...
    axes.plot(time_index, body_torque[0], label= 'L')
    axes.plot(time_index, body_torque[1], label= 'M')
    axes.plot(time_index, body_torque[2], label= 'N')
    ...
    axes.plot(time_index, angle_vel[0], label= 'p')
    axes.plot(time_index, angle_vel[1], label= 'q')
    axes.plot(time_index, angle_vel[2], label= 'r')
    axes.set_title('Angular Velocity')
    axes.set_xlabel('time (s)')
    axes.set_ylabel('angular velocity (grad/s)')
    axes.legend()
